...
hdlr = logging.FileHandler('./log/log_{}.log'.format(currenttime))
...
def train(traindatadir, valdatadir, traindatacsv,valdatacsv,device,save_model_filename,lambda_):
    EPOCH = 100
    printout_steps = 50
    eval_steps = 200
    lr_steps = 300
    ...
    def updateBN():
        """
        since L1 norm is not differentiable, we use semi gradient descent here
        """
        for m in cnnmodel.modules():
            if isinstance(m,nn.BatchNorm2d):
                m.weight.grad.data.add_(lambda_*torch.sign(m.weight.data))
...
if __name__ == '__main__':
    ...
    #screen 14315
    save_model_filename = config.get('SaveModel','model_before_prune')
    lambda_ = float(config.get('Parameters','lambda_'))
    ...
    train(datapath,val_datapath,csvpath,val_csv_path,device,save_model_filename,lambda_)
